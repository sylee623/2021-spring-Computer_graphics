...
def render(t):
    ...
    R1 = np.identity(4)
    R1[:3,:3] = np.array([[np.cos(t), 0,  np.sin(t)],
                   [0, 1, 0],
                   [-np.sin(t),0,np.cos(t)]])
    J1 = R1
    ...
    R2[:3, :3 ] = np.array([[1,0,0],
                   ...
                   [0, np.sin(t), np.cos(t)]])
    tmp = np.identity(4)
    tmp[:3, :3] = np.array([[np.cos(t), -np.sin(t), 0],
                   [np.sin(t), np.cos(t), 0],
                   [0,0,1]])
    R2 = R2 @ tmp
    T1 = np.identity(4)
    ...
    glBegin(GL_LINES)
    glVertex3fv(E[:-1])
    glVertex3fv(E[:-1] + difference[:-1] * 10)
    glEnd()
    glPopMatrix()
